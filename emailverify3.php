                    ...
                    if($_SERVER['REQUEST_METHOD'] == 'POST')
                    {
                        ...
                        else
                        {
                            ...
                            if($otp == $_SESSION['code'])
                            {
                                ...
                                $sql = "INSERT INTO `login` (`login_email`, `login_password`, `login_username`, `login_role`) VALUES ('$email', '$pass', '$name', 'e')";
                                $result = mysqli_query($conn, $sql);
                                $sql = "SELECT * FROM `login` WHERE login_email = '$email'";
                                $result = mysqli_query($conn, $sql);
                                $row = mysqli_fetch_array($result);
                                $no = $row['login_srno'];
                                $sql = "INSERT INTO employee (`employe_name`, `employee_contact`, `preferred_pin`, `login_srno`) VALUES ('$realname', '$phone', '$pin', '$no')";
                                $result = mysqli_query($conn, $sql);
                                session_unset();
                                session_destroy();
