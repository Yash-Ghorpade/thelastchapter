    ...
    <div id="main-wrapper">
        ...
        ***********************************-->
        <div class="content-body">
              search by email :
            <form class='d-none d-sm-inline-block me-auto ms-md-3 my-2 my-md-0 mw-100 navbar-search' method='post' action='customerHistory2.php' style = 'display:flex;justify-content:center'>
            ...
            {
                $inp = $_POST['search'];
                $sql = "select * from issue inner join customer on customer.customer_srno = issue.customer_srno inner join book on issue.book_srno = book.book_srno inner join login on customer.login_srno = login.login_srno where login_email = '$inp';";
                $result = mysqli_query($conn, $sql);
                ...
                echo "<tr>
                        ...
                        if($display['issue_date'] != NULL)
                        {
                            echo "<td>".$display['issue_date']."</td>";
                        }
                        else
                        {
                            echo "<td>Yet to be issued</td>";
                        }
                        if($display['issue_status'] == 'c')
                        {
                            echo "<td>".$date2."</td>";
                        }
                        else
                        {
                            echo "<td>Under issue/ not issued</td>";
                        }
                    echo "</tr>";
                $sr=$sr + 1;
                }
